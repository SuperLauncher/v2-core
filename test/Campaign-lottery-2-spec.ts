...
makeSuite('Campaign-lottery-2', (testEnv: TestEnv) => {
	var main: Campaign;
	...
	it('Should able refund excess & refund fund when campaign is cancelled', async () => {
		...
		//user buy 0.1 bnb
		await main.connect(user1.signer).buyTokens(ethers.utils.parseEther("0.05"), { value: ethers.utils.parseEther("0.05") })
		await main.connect(user1.signer).buyTokens(ethers.utils.parseEther("0.05"), { value: ethers.utils.parseEther("0.05") })
		// await main.connect(user1.signer).buyTokens(ethers.utils.parseEther("0.05"), { value: ethers.utils.parseEther("0.05") })
	...
	it('Should able to claim back the refund', async () => {
		...
		if (!resultUser1.wonLottery && !resultUser1.wonOverSub) {
			...
			//call return refund to get back 0.1 bnb
			await main.connect(user1.signer).returnFund();
			const bnbBalanceAT2 = await ethers.provider.getBalance(user1.address);
			const delta2 = bnbBalanceAT2.sub(bnbBalanceAT) > ethers.utils.parseEther("0.09");
