...
makeSuite('Campaign-flow-interval-erc20-test', (testEnv: TestEnv) => {
	var main: Campaign;
	...
	it('Should able to claim interval', async () => {
		...
		const user1 = users[0]
		//approval
		await BUSD.connect(user1.signer).mint(ethers.utils.parseEther("1"));
		await BUSD.connect(user1.signer).approve(main.address, ethers.utils.parseEther("1"));
